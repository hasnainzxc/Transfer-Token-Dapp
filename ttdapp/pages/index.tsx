...
import { Box, Button, Container, Flex, Heading, SimpleGrid, Stack, Text, useBreakpointValue } from "@chakra-ui/react";
...
const Home: NextPage = () => {
  ...
  return (
    <Container maxW="1440px">
      <Flex direction={isMobile ? "column" : "row"} h={isMobile ? "auto" : "75vh"} borderRadius={20} px={isMobile ? 4 : 20} py={isMobile ? 8 : 0}>
        <Flex flexDirection="column" justifyContent="center" w={isMobile ? "100%" : "60%"} pr={isMobile ? 0 : 10} mb={isMobile ? 4 : 0}>
          <Stack spacing={4}>
            {/* <Heading fontSize="xl">Transfer App</Heading> */}
            <Heading fontSize="6xl">Send tokens to your friends and family with ease.</Heading>
            <Text fontSize="xl">
            ...
            <Link href="/transfer">
              <Button w={isMobile ? "100%" : "80%"}>Make a Transfer</Button>
            </Link>
          </Stack>
        </Flex>
        <Box flexShrink={0} ml={isMobile ? 0 : 8}>
          <MediaRenderer src={HERO_IMAGE_URL} height={isMobile ? "auto" : "100%"} width={isMobile ? "100%" : "auto"} />
        </Box>
      </Flex>
